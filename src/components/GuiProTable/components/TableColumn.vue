...
<script setup lang="jsx" name="TableColumn">
    ...
    import { filterEnum, formatValue, handleProp, handleRowAccordingToProp } from '@/utils/modules/components'
    ...
    const RenderTableColumn = (item) => {
        return (
            <>
                {item.isShow && (
                    <el-table-column {...item} align={item.align ?? 'left'} showOverflowTooltip={item.showOverflowTooltip ?? item.prop !== 'operation'}>
                        {{
                            default: (scope) => {
