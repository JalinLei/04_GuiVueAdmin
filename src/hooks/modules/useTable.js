...
export const useTable = (api, initParam = {}, isPageable = true, dataCallBack = null, requestError) => {
    ...
    const getTableList = async () => {
        ...
        try {
            ...
            state.value.tableData = isPageable ? (data.list || []) : data;
            // 解构后台返回的分页数据 (如果有分页更新分页信息)
            if (isPageable) state.value.pageable.total = data.total || 0;
            // 简单判断data是否树结构
            if ((data.list || []).find(item => item?.children || item?.child || item?._children)) isTreeData.value = true;
        } catch (error) {
            console.log({error});
